  ...
  return db.prepare(`
    SELECT t.tag, COUNT(*) n,
      (SELECT media_id FROM media_tags t2 JOIN media m2 ON m2.id = t2.media_id
       WHERE t2.tag = t.tag AND m2.deleted_at IS NULL AND m2.archived = 0 AND m2.locked = 0
...
function thingItems(tag) {
  return db.prepare(`
    ...
    WHERE mt.tag = ? AND m.deleted_at IS NULL AND m.archived = 0 AND m.locked = 0
...
module.exports = { newJob, getJob, current, runIndex, runRetag, search, stats, warm, things, thingItems };
