...
function spawn() {
  ...
  worker.on('message', (m) => {
    ...
    if (m.ev === 'done' && job && m.id === job.id) {
      ...
      Promise.resolve(job._log).then((id) => joblog.finish(id, 'done', job.embedded + ' imagini' + (job.ocred ? ', ' + job.ocred + ' OCR' : '')));
      return;
    }
  ...
  worker.on('exit', (code) => {
    ...
    pending.clear();
    if (job && !job.finishedAt) { job.phase = 'error'; job.error = 'worker oprit (cod ' + code + ')'; Promise.resolve(job._log).then((id) => joblog.finish(id, 'error', job.error)); }
  });
}
...
// ─── Indexare ────────────────────────────────────────────────────────────
let jseq = 0;
function newJob(kind) { job = { id: 'idx' + (++jseq), phase: 'starting', total: 0, done: 0, embedded: 0, ocred: 0, startedAt: new Date().toISOString() }; job._log = joblog.start(kind || 'Indexare'); /* Promise<id> */ return job; }
...
async function search(q, { limit = 120 } = {}) {
  ...
  // 1. semantic
  const vecRows = await db.prepare(`
...
async function nearDuplicates(th = 0.955, cap = 6000) {
  const rows = await db.prepare(`
...
async function stats() {
  const embed = (await db.prepare('SELECT COUNT(*) n FROM media_embed').get()).n;
  const ocr = (await db.prepare('SELECT COUNT(*) n FROM media WHERE ocr_done = 1').get()).n;
  const total = (await db.prepare('SELECT COUNT(*) n FROM media WHERE deleted_at IS NULL').get()).n;
