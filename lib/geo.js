...
async function reverse(lat, lon) {
  ...
  const hit = await db.prepare('SELECT place, city, country FROM geocache WHERE cell = ?').get(key);
  if (hit) return hit;
  ...
  await db.prepare('REPLACE INTO geocache (cell, place, city, country, at) VALUES (?, ?, ?, ?, ?)')
    .run(key, place, city, country, new Date().toISOString());
  return row;
}

// Geocodare directă (nume loc -> coordonate), pentru adăugarea manuală a
// locației la poze fără GPS. Aceeași politețe (max 1 cerere/sec) ca reverse().
async function search(q) {
  ...
  last = Date.now();
  try {
    const u = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=jsonv2&limit=6&accept-language=ro`;
    const r = await fetch(u, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(15000) });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const j = await r.json();
    return j.map((x) => ({ label: x.display_name, lat: Number(x.lat), lon: Number(x.lon) }));
  } catch { return []; }
}
...
async function backfillPlaces(limit = 400) {
  ...
  for (const r of rows) {
    ...
    await db.prepare('UPDATE media SET place = ?, city = ?, country = ?, place_done = 1 WHERE id = ?')
  ...
  return db.prepare(`
    SELECT COALESCE(place, city, country) AS place,
           ...
           (SELECT id FROM media m2 WHERE COALESCE(m2.place, m2.city, m2.country) = COALESCE(m.place, m.city, m.country)
              AND m2.deleted_at IS NULL AND m2.archived = 0 AND m2.locked = 0 AND m2.is_live_motion = 0 ORDER BY COALESCE(m2.taken_at, m2.created_at) DESC LIMIT 1) AS sampleId
    FROM media m
...
module.exports = { reverse, search, backfillPlaces, placesSummary };
