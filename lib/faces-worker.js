...
function assignCluster(db, mediaId, desc) {
  const clusters = db.prepare('SELECT id FROM face_clusters').all();
  let best = null, bestD = 1e9;
  for (const c of clusters) {
    const already = db.prepare('SELECT 1 FROM faces WHERE cluster_id = ? AND media_id = ?').get(c.id, mediaId);
    if (already) continue;
    const mem = db.prepare('SELECT descriptor FROM faces WHERE cluster_id = ? LIMIT 12').all(c.id);
    let md = 1e9;
    for (const m of mem) md = Math.min(md, dist(desc, b2f(m.descriptor)));
    if (md < bestD) { bestD = md; best = c.id; }
  }
  if (best && bestD < JOIN_TH) {
    db.prepare('UPDATE face_clusters SET n = n + 1 WHERE id = ?').run(best);
    return best;
  }
  const id = crypto.randomUUID();
  db.prepare('INSERT INTO face_clusters (id, name, cover_face_id, n, created_at) VALUES (?, NULL, NULL, 1, ?)')
...
async function runIndex(msg) {
  ...
  const rows = db.prepare("SELECT id, stored_name FROM media WHERE type = 'image' AND deleted_at IS NULL AND faces_done = 0").all();
  ...
  for (const r of rows) {
    ...
    try {
      if (fs.existsSync(file)) {
        ...
        for (const d of res) {
          ...
          const cid = db.transaction(() => {
            const c = assignCluster(db, r.id, desc);
            insFace.run(fid, r.id, c, box, d.detection.score, f2b(desc), new Date().toISOString());
            setCover.run(fid, c);
            return c;
          })();
          faces++;
      ...
      db.prepare('UPDATE media SET faces_done = 1 WHERE id = ?').run(r.id);
    } catch (e) { db.prepare('UPDATE media SET faces_done = 1 WHERE id = ?').run(r.id); }
    done++;
