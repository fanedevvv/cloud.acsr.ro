...
// și creează un album cu același nume în galerie.
const fs = require('fs');
...
async function fetchPage(url) {
  const r = await fetch(url, { redirect: 'follow', headers: { 'User-Agent': UA, 'Accept-Language': 'ro,en;q=0.8' }, signal: AbortSignal.timeout(20000) });
...
function parseAlbum(html) {
  ...
  if (m) name = m[1].replace(/\s*[-–|]\s*Google (Photos|Fotos).*$/i, '').replace(/&amp;/g, '&').trim();

  // fiecare element apare ca ["<url lh3>",latime,inaltime]
  const items = [];
  const seen = new Set();
  const re = /\["(https:\/\/lh3\.googleusercontent\.com\/[^"\\]+)",(\d{2,5}),(\d{2,5})\]/g;
  let x;
...
async function grabOne(it) {
  // întâi încearcă varianta video (merge doar pentru clipuri reale)
  try {
    const rv = await fetch(it.base + '=dv', { redirect: 'follow', headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(120000) });
  ...
  // altfel: poza originală
  const ri = await fetch(it.base + '=d', { redirect: 'follow', headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(120000) });
  ...
  try {
    ...
    const workers = new Array(3).fill(0).map(async () => {
      while (i < items.length && i < LIMIT) {
        ...
        const it = items[idx];
        try {
          const g = await grabOne(it);
          const tmp = path.join(workDir, 'g' + idx + '.' + g.ext);
          fs.writeFileSync(tmp, g.buf);
          const res = await ingest({ srcPath: tmp, originalName: 'gphotos-' + (idx + 1) + '.' + g.ext, mime: g.mime, uploaderId: uploaderId || null });
          if (res && res.id) {
    ...
    if (!mediaIds.length) throw new Error('nu s-a putut descărca niciun fișier');
    ...
    if (found) aid = found.id;
      ...
      await db.prepare('INSERT INTO albums (id, name, created_at, owner_id, owner_name) VALUES (?, ?, ?, ?, ?)')
        .run(aid, nm, new Date().toISOString(), uploaderId || null, 'Cont');
    }
    const now = new Date().toISOString();
    const ins = db.prepare('INSERT IGNORE INTO album_items (album_id, media_id, added_at) VALUES (?, ?, ?)');
    for (const mid of mediaIds) await ins.run(aid, mid, now);
    job.albumId = aid;
