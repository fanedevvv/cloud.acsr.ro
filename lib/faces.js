...
function spawn() {
  ...
  worker.on('message', (m) => {
    ...
    if ((m.ev === 'progress' || m.ev === 'done') && job && m.id === job.id) {
      ...
      if (m.ev === 'done') { job.finishedAt = new Date().toISOString(); Promise.resolve(job._log).then((id) => joblog.finish(id, 'done', job.faces + ' fețe, ' + job.people + ' persoane')); }
      return;
    }
  ...
  worker.on('exit', (code) => {
    ...
    pending.clear();
    if (job && !job.finishedAt) { job.phase = 'error'; job.error = 'worker oprit (' + code + ')'; Promise.resolve(job._log).then((id) => joblog.finish(id, 'error', job.error)); }
  });
}
