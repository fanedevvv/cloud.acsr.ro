...
function spawn() {
  ...
  worker.on('message', (m) => {
    ...
    if ((m.ev === 'progress' || m.ev === 'done') && job && m.id === job.id) {
      ...
      if (m.ev === 'done') job.finishedAt = new Date().toISOString();
      return;
    }
  ...
  worker.on('exit', (code) => {
    ...
    pending.clear();
    if (job && !job.finishedAt) { job.phase = 'error'; job.error = 'worker oprit (' + code + ')'; }
  });
}
const ensure = () => { if (!worker) spawn(); return worker; };

let jseq = 0;
function newJob() { job = { id: 'face' + (++jseq), phase: 'starting', total: 0, done: 0, faces: 0, people: 0, startedAt: new Date().toISOString() }; return job; }
