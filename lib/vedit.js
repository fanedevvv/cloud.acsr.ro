...
async function runSlideshow(job, images, opts) {
  try {
    ...
    const total = (secs * images.length).toFixed(2);
    ...
    if (hasAudio) parts.push(`[${images.length}:a]afade=t=out:st=${Math.max(0, total - 1.2).toFixed(2)}:d=1.2,apad[a]`);
    args.push('-filter_complex', parts.join(';'), '-map', '[v]');
    if (hasAudio) args.push('-map', '[a]', '-c:a', 'aac', '-b:a', '160k', '-t', String(total));
    args.push('-c:v', 'libx264', '-crf', '26', '-preset', 'ultrafast', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', out);
    job.phase = 'processing';
...
// ─── Animație -> WebP animat, în buclă (ca „Animation" din Google Photos) ──
async function runAnimation(job, images, opts) {
  const frameDir = fs.mkdtempSync(path.join(media.TMP_DIR, 'anim-'));
  try {
    const SIZE = 480;
    job.phase = 'processing';
    let i = 0;
    for (const src of images) {
      ...
      await sharp(src, { failOn: 'none', limitInputPixels: 2_000_000_000 })
        .rotate().resize(SIZE, SIZE, { fit: 'cover' }).png().toFile(out);
      i++;
    }
    const out = path.join(media.TMP_DIR, 'anim-' + job.id + '.webp');
    await runFfmpeg([
      '-y', '-framerate', '2', '-i', path.join(frameDir, 'f%03d.png'),
      '-vcodec', 'libwebp', '-lossless', '0', '-q:v', '75', '-loop', '0', '-an', '-vsync', '0', out,
    ], 10 * 60 * 1000);
    if (!fs.existsSync(out) || fs.statSync(out).size === 0) throw new Error('rezultat gol');
    const r = await media.ingest({ srcPath: out, originalName: 'animatie-' + Date.now() + '.webp', mime: 'image/webp' });
    job.phase = 'done'; job.mediaId = r.id; job.finishedAt = new Date().toISOString();
    joblog.finish(await job._log, 'done', images.length + ' poze animate');
  } catch (e) {
    job.phase = 'error';
    job.error = (e && e.message) || String(e);
    joblog.finish(await job._log, 'error', job.error);
...
module.exports = { newJob, getJob, runVideoEdit, runSlideshow, runAnimation, frameBuffer, saveFrame, available: !!FFMPEG };
