...
addCol('live_video_id', 'live_video_id TEXT');
addCol('is_live_motion', 'is_live_motion INTEGER NOT NULL DEFAULT 0');
...
if (!userCols.has('google_id')) db.exec('ALTER TABLE users ADD COLUMN google_id TEXT');
if (!userCols.has('email')) db.exec('ALTER TABLE users ADD COLUMN email TEXT');
...
if (!albumCols.has('owner_id')) db.exec('ALTER TABLE albums ADD COLUMN owner_id TEXT');
if (!albumCols.has('owner_name')) db.exec('ALTER TABLE albums ADD COLUMN owner_name TEXT');
