...
async function init() {
  ...
  await addCol('face_clusters', 'hidden', 'hidden TINYINT(1) NOT NULL DEFAULT 0');
  await addCol('face_clusters', 'is_pet', 'is_pet TINYINT(1) NOT NULL DEFAULT 0');
  await addCol('users', 'partner_id', 'partner_id VARCHAR(36)');
  ...
  await addCol('albums', 'share_expires_at', 'share_expires_at VARCHAR(40)');
  await addCol('albums', 'auto_person_cluster_id', 'auto_person_cluster_id VARCHAR(36)');
  await addCol('media', 'share_expires_at', 'share_expires_at VARCHAR(40)');
  await addCol('media', 'dhash', 'dhash CHAR(16)');
  // Redare video în browser: 0=necunoscut, 1=originalul e ok (H.264), 2=are <id>.play.mp4 (transcodat), 3=eșuat
  await addCol('media', 'play_ready', 'play_ready TINYINT NOT NULL DEFAULT 0');
  await addIndex('media', 'idx_media_dhash', 'INDEX idx_media_dhash (dhash)');
  ...
  await db.exec(`
    CREATE TABLE IF NOT EXISTS album_collaborators (
      ...
      CONSTRAINT fk_collab_album FOREIGN KEY (album_id) REFERENCES albums(id) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `);
  ...
  // Cine a deschis un link de partajare (ca „membrii" din Google Photos)
  await db.exec(`
    CREATE TABLE IF NOT EXISTS share_views (
      token      VARCHAR(64) NOT NULL,
      viewer_key VARCHAR(64) NOT NULL,
      name       VARCHAR(150),
      first_at   VARCHAR(40) NOT NULL,
      last_at    VARCHAR(40) NOT NULL,
      PRIMARY KEY (token, viewer_key)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  `);
  await addIndex('share_views', 'idx_share_views_token', 'INDEX idx_share_views_token (token, last_at)');

  // Albume „ascunse" (create doar ca să partajezi o selecție de poze)
  await addCol('albums', 'hidden', 'hidden TINYINT(1) NOT NULL DEFAULT 0');
  await addCol('albums', 'selection_key', 'selection_key VARCHAR(80)');
  ...
  // Jurnal de activitate: cine a făcut ce (audit trail)
  await db.exec(`
  ...
  `);
  await addIndex('audit_log', 'idx_audit_created', 'INDEX idx_audit_created (created_at)');
}
