...
// Baza de date: același pool mysql2 (conexiuni proprii — proces separat)
...
async function runIndex(msg) {
  ...
  const insVec = db.prepare('REPLACE INTO media_embed (media_id, vec, done_at) VALUES (?, ?, ?)');
  const setOcr = db.prepare('UPDATE media SET ocr_text = ?, ocr_done = 1 WHERE id = ?');
  const insTag = db.prepare('REPLACE INTO media_tags (media_id, tag, score) VALUES (?, ?, ?)');
  ...
  for (const r of todo) {
    ...
    try {
      if (!r.hasVec && fs.existsSync(img)) {
        ...
        await insVec.run(r.id, Buffer.from(v.buffer, v.byteOffset, v.byteLength), new Date().toISOString());
        embedded++;
        if (concepts && r.type === 'image') {
          const scored = concepts.map(([tag, cv]) => [tag, cos(v, cv)]).sort((a, b) => b[1] - a[1]);
          await db.prepare('DELETE FROM media_tags WHERE media_id = ?').run(r.id);
          for (const [tag, sc] of scored.slice(0, 4)) if (sc >= 0.235) await insTag.run(r.id, tag, sc);
        }
      }
      if (ocr && r.type === 'image' && !r.ocr_done && fs.existsSync(img)) {
        await setOcr.run((await ocrImage(img)) || null, r.id);
        ocred++;
      }
    } catch (e) { /* sări peste */ }
    done++;
...
async function runRetag(msg) {
  ...
  const rows = await db.prepare('SELECT media_id, vec FROM media_embed').all();
  ...
  for (const r of rows) {
    try {
      ...
      const scored = concepts.map(([tag, cv]) => [tag, cos(v, cv)]).sort((a, b) => b[1] - a[1]);
      await del.run(r.media_id);
      for (const [tag, sc] of scored.slice(0, 4)) if (sc >= 0.235) { await insTag.run(r.media_id, tag, sc); tagged++; }
    } catch {}
    done++;
