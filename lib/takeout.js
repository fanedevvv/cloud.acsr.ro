...
function newJob() {
  ...
  const job = {
    ...
    errors: [], startedAt: Date.now(), finishedAt: null, _log: null,
  };
  jobs.set(id, job);
...
async function runImport(zipPath, job) {
  ...
  try {
    job._log = await joblog.start('Import Google Takeout');
    job.phase = 'reading';
    ...
    const insItem = db.prepare('INSERT IGNORE INTO album_items (album_id, media_id, added_at) VALUES (?, ?, ?)');
    for (const entry of albumsByDir.values()) {
      const nm = String(entry.name || '').trim().slice(0, 120);
      if (!nm || !entry.mediaIds.length) continue;
      let aid;
      const found = await findAlbum.get(nm);
      if (found) aid = found.id;
      else { aid = crypto.randomUUID(); await insAlbum.run(aid, nm, new Date().toISOString()); }
      const now = new Date().toISOString();
      for (const mid of entry.mediaIds) await insItem.run(aid, mid, now);
      job.albums++;
    }

    job.phase = 'done';
    await joblog.finish(job._log, 'done', '+' + job.added + ' poze, ' + job.albums + ' albume');
  } catch (e) {
    job.phase = 'error';
    job.errors.push('fatal: ' + (e && e.message ? e.message : e));
    await joblog.finish(job._log, 'error', job.errors[job.errors.length-1]);
  } finally {
