...
async function ingest(opts) {
  ...
  const hash = await sha256File(srcPath).catch(() => null);
  if (hash) {
    const dup = db.prepare('SELECT id FROM media WHERE sha256 = ? AND deleted_at IS NULL').get(hash);
    if (dup) { fs.rmSync(srcPath, { force: true }); return { id: dup.id, duplicate: true }; }
  }

  const id = crypto.randomUUID();
  const ext = extOf(originalName) || (kind === 'image' ? 'jpg' : 'mp4');
  ...
  if (kind === 'image') {
    try {
      const meta = await sharp(srcPath, { failOn: 'none', limitInputPixels: 2_000_000_000 }).metadata();
  ...
  const row = {
    id, type: kind,
    mime: mime || (kind === 'image' ? 'image/jpeg' : 'video/mp4'),
    stored_name: storedName, original_name: originalName || null,
