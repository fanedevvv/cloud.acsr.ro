...
async function ingest(opts) {
  ...
  let dhash = null;
  if (kind === 'image') {
    dhash = await computeDhash(work);
    if (dhash) {
      let ar = 0;
      try { const m = await sharp(work, { failOn: 'none', limitInputPixels: 2_000_000_000 }).metadata(); if (m.width && m.height) ar = m.width / m.height; } catch { /* */ }
      const cands = await db.prepare(
        "SELECT id, dhash, width, height FROM media WHERE type = 'image' AND deleted_at IS NULL AND dhash IS NOT NULL"
      ).all();
      for (const c of cands) {
        const car = c.width && c.height ? c.width / c.height : 0;
        const aspectOk = !ar || !car || Math.abs(ar - car) < 0.04;
        if (aspectOk && hammingHex(dhash, c.dhash) <= 3) {
          fs.rmSync(work, { force: true });
          return { id: c.id, duplicate: true };
        }
      }
    }
  }
...
async function processUpload(file, uploaderId) {
  return ingest({ srcPath: file.path, originalName: file.originalname, mime: file.mimetype, uploaderId: uploaderId || null });
}
...
async function backfillDhash(limit = 4000) {
  ...
  // După ce toate au dhash, elimină duplicatele perceptuale existente.
  const all = await db.prepare(
    "SELECT id, dhash, width, height, favorite, created_at FROM media WHERE type = 'image' AND deleted_at IS NULL AND dhash IS NOT NULL ORDER BY created_at ASC"
  ).all();
  const kept = [];
  let removed = 0;
  for (const m of all) {
    ...
    const idx = kept.findIndex((k) => {
      const kar = k.width && k.height ? k.width / k.height : 0;
      const aspectOk = !ar || !kar || Math.abs(ar - kar) < 0.04;
      return aspectOk && hammingHex(m.dhash, k.dhash) <= 3;
    });
    if (idx === -1) { kept.push(m); continue; }
    const k = kept[idx];
    const preferM = !!m.favorite && !k.favorite;   // altfel păstrăm primul (cel mai vechi)
    const winner = preferM ? m : k;
    const loser = preferM ? k : m;
    if (preferM) kept[idx] = m;
    await db.prepare('UPDATE media SET deleted_at = ? WHERE id = ? AND deleted_at IS NULL')
      .run(new Date().toISOString(), loser.id);
    try {
      await db.prepare('UPDATE IGNORE album_items SET media_id = ? WHERE media_id = ?').run(winner.id, loser.id);
      await db.prepare('DELETE FROM album_items WHERE media_id = ?').run(loser.id);
    } catch { /* */ }
    removed++;
  }
  if (removed) console.log(`duplicate perceptuale mutate în coș: ${removed}`);
}
