...
// Recompresie webp a unei singure poze, după ce a fost deja ingerată și
// răspunsul de upload a plecat — evită să blocheze utilizatorul ~5s/poză
// pe acest CPU. Aceeași logică pe care „Optimizează spațiul" o rulează în bloc.
async function backgroundOptimizeOne(id, storedName) {
  const src = path.join(ORIGINAL_DIR, storedName);
  if (!fs.existsSync(src)) return;
  let opt;
  try { opt = await optimizeImage(src); } catch { return; }
  if (!opt || opt.savedBytes <= 0) { if (opt && opt.outPath) fs.rmSync(opt.outPath, { force: true }); return; }
...
async function ingest(opts) {
  ...
  // fundal, după răspuns (vezi backgroundOptimizeOne mai jos).
  const work = src;
  ...
  if (kind === 'image' && OPT_UPLOADS !== 'off') {
    backgroundOptimizeOne(id, storedName).catch(() => {});
  }
