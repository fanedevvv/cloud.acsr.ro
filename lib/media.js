...
function tryPairLive(id, kind, originalName, createdAt) {
  ...
  db.prepare('UPDATE media SET live_video_id = ? WHERE id = ?').run(motion, still);
  db.prepare('UPDATE media SET is_live_motion = 1 WHERE id = ?').run(motion);
}
...
async function ingest(opts) {
  ...
  tryPairLive(id, kind, originalName, createdAt);
...
async function backfillExif() {
  ...
  for (const r of rows) {
    const p = path.join(ORIGINAL_DIR, r.stored_name);
    if (!fs.existsSync(p)) { db.prepare('UPDATE media SET exif_done = 1 WHERE id = ?').run(r.id); continue; }
    let ex = { lat: null, lon: null, camera: null, lens: null, iso: null, fNumber: null, exposure: null, focal: null };
    let fmt = null;
    try {
      if (r.type === 'image') {
