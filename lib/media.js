...
// dHash perceptual pe 64 de biți (rezistent la recompresie/redimensionare).
// Se folosește pentru a elimina automat pozele „identice" reîncărcate,
// chiar dacă browserul le-a re-encodat înainte de upload (bytes diferiți).
async function computeDhash(src) {
  try {
    const px = await sharp(src, { failOn: 'none', limitInputPixels: 2_000_000_000 })
...
// Recompresie webp a unei singure poze (aceeași logică pe care
// „Optimizează spațiul" o rulează în bloc).
async function optimizeOne(id, storedName) {
  const src = path.join(ORIGINAL_DIR, storedName);
  if (!fs.existsSync(src)) return false;
  let opt;
  try { opt = await optimizeImage(src); } catch { return false; }
  if (!opt || opt.savedBytes <= 0) { if (opt && opt.outPath) fs.rmSync(opt.outPath, { force: true }); return false; }
  const newStored = id + opt.ext;
  const newPath = path.join(ORIGINAL_DIR, newStored);
  fs.renameSync(opt.outPath, newPath);
  if (newStored !== storedName) fs.rmSync(src, { force: true });
  const hash = await sha256File(newPath).catch(() => null);
  const size = fs.statSync(newPath).size;
  await remakeThumb(id, newPath, 'image').catch(() => {});
  await db.prepare('UPDATE media SET stored_name=?, mime=?, size=?, sha256=? WHERE id=?')
    .run(newStored, opt.mime, size, hash, id);
  return true;
}

// Recompresie lentă în fundal: câteva poze neoptimizate pe rulare, ca să nu
// concureze niciodată cu upload-uri active pe CPU-ul ăsta slab.
async function backfillOptimize(limit = 3) {
  if (OPT_UPLOADS === 'off') return;
  const rows = await db.prepare(
    "SELECT id, stored_name FROM media WHERE type = 'image' AND deleted_at IS NULL " +
    "AND mime NOT IN ('image/webp', 'image/avif') LIMIT ?"
  ).all(limit);
  let n = 0;
  for (const r of rows) {
    try { if (await optimizeOne(r.id, r.stored_name)) n++; } catch { /* skip */ }
  }
  if (n) console.log(`optimizare fundal: ${n} poze recomprimate`);
}
...
async function ingest(opts) {
  ...
  // Recompresia (optimizeImage) NU se mai face aici, sincron — pe CPU-ul ăsta
  // costă ~5s per poză și dubla asta timpul de upload degeaba. Se face în
  // fundal, după răspuns (vezi backfillOptimize()).
  const work = src;
  ...
  // Dedup perceptual pentru imagini: dacă aceeași poză a fost deja încărcată
  // (chiar re-encodată de browser), nu mai păstrăm un al doilea exemplar.
  // dhash se calculează pentru unealta „Găsește duplicate", dar NU se mai
  // șterge nimic automat pe baza lui (un dHash de 64 de biți poate confunda
  // cadre dintr-o rafală). „Aceeași poză de 2 ori" = același sha256 (mai sus).
  let dhash = null;
  if (kind === 'image') dhash = await computeDhash(work);
  ...
  if (kind === 'image') {
    try {
      ...
      await sharp(work, { failOn: 'none', limitInputPixels: 2_000_000_000 })
        ...
        .webp({ quality: 78 }).toFile(thumbPath);
      hasThumb = 1;
      // preview-ul de 1600px e doar pt. zoom în lightbox — se face în fundal
      // (backfillPreviews, rulează periodic), nu blochează răspunsul de upload.
  ...
  const row = {
    ...
    uploader_id: opts.uploaderId || null,
    dhash: dhash || null,
  };
  await db.prepare(`
    INSERT INTO media (id, type, mime, stored_name, original_name, width, height, size, duration,
      ...
      lat, lon, camera, lens, iso, f_number, exposure, focal, exif_done, kind_auto, uploader_id, dhash)
  ...
  // Recompresia NU se declanșează per-upload (ar aglomera CPU-ul slab și ar
  // încetini restul pozelor din lot). Se face lent, în fundal — backfillOptimize().
...
async function processUpload(file, uploaderId) {
  return ingest({ srcPath: file.path, originalName: file.originalname, mime: file.mimetype, uploaderId: uploaderId || null, perceptualDedup: true });
}
...
// Codec-ul video e redabil direct in browsere obisnuite? (H.264 8-bit da; HEVC/H.265, VP9-in-mp4, AV1, 10-bit — nu)
function videoWebCodec(file) {
  if (!FFPROBE) return { ok: true, codec: '?' };
  try {
    const out = execFileSync(FFPROBE, ['-v', 'error', '-select_streams', 'v:0',
      '-show_entries', 'stream=codec_name,pix_fmt', '-of', 'default=nw=1:nk=1', file],
      { encoding: 'utf8', timeout: 20000 }).trim().split(/\s+/);
    const codec = (out[0] || '').toLowerCase();
    const pix = (out[1] || '').toLowerCase();
    const ok = (codec === 'h264') && !pix.includes('10le') && !pix.includes('12le') && !pix.includes('p10') && !pix.includes('p12');
    return { ok, codec };
  } catch { return { ok: true, codec: '?' }; }
}

// Fac o varianta H.264 720p redabila in browser (originalul ramane pt. descarcare).
function transcodeWeb(src, dest) {
  return new Promise((resolve) => {
    if (!FFMPEG) return resolve(false);
    execFile(FFMPEG, [
      '-y', '-i', src,
      ...
      dest,
    ], { timeout: 30 * 60 * 1000, maxBuffer: 1 << 22 }, (err) => {
      if (err) { try { fs.rmSync(dest, { force: true }); } catch {} return resolve(false); }
      try { resolve(fs.existsSync(dest) && fs.statSync(dest).size > 1000); }
      catch { resolve(false); }
    });
  });
}

// Pregateste cate un clip pentru redare web (fundal, unul odata — CPU slab).
async function backfillTranscode(limit = 1) {
  if (!FFMPEG) return;
  const rows = await db.prepare(
    "SELECT id, stored_name FROM media WHERE type = 'video' AND deleted_at IS NULL AND play_ready = 0 LIMIT ?"
  ).all(limit);
  for (const r of rows) {
    const src = path.join(ORIGINAL_DIR, r.stored_name);
    if (!fs.existsSync(src)) { await db.prepare('UPDATE media SET play_ready = 3 WHERE id = ?').run(r.id); continue; }
    const web = videoWebCodec(src);
    if (web.ok) { await db.prepare('UPDATE media SET play_ready = 1 WHERE id = ?').run(r.id); continue; }
    const dest = path.join(ORIGINAL_DIR, r.id + '.play.mp4');
    console.log(`transcodez pentru redare web: ${r.id} (${web.codec})`);
    const ok = await transcodeWeb(src, dest);
    await db.prepare('UPDATE media SET play_ready = ? WHERE id = ?').run(ok ? 2 : 3, r.id);
    if (ok) console.log(`  gata: ${r.id}.play.mp4`);
  }
}
...
// Completează dhash pentru imaginile fără (fundal, la pornire) + șterge
// automat duplicatele perceptuale rămase din trecut (păstrează 1 exemplar).
async function backfillDhash(limit = 4000) {
  const rows = await db.prepare(
    "SELECT id, stored_name FROM media WHERE type = 'image' AND dhash IS NULL AND deleted_at IS NULL LIMIT ?"
  ).all(limit);
  let done = 0;
  for (const r of rows) {
    const p = path.join(ORIGINAL_DIR, r.stored_name);
    if (!fs.existsSync(p)) continue;
    const dh = await computeDhash(p);
    if (dh) { await db.prepare('UPDATE media SET dhash = ? WHERE id = ?').run(dh, r.id); done++; }
  }
  if (done) console.log(`dhash calculat pentru ${done} imagini`);
  // NU se mai șterge nimic automat pe baza dhash-ului: un dHash de 64 de biți
  // poate confunda cadre dintr-o rafală. Dedup-ul „aceeași poză de 2 ori" e
  // acoperit de sha256 (ingest) + verificarea perceptuală doar la upload din
  // browser. dhash rămâne calculat pentru unealta „Găsește duplicate".
}
